...
It is loaded by `/hi-seamlex` at the start of the session, after the `seamlex-portal-memory` page, and
...
Every `{{PLACEHOLDER}}` below resolves from the `seamlex-portal-memory` page `/hi-seamlex` loaded —
`{{JIRA_PROJECT}}`, `{{CONF_PARENT}}`, `{{CONF_SCOPE_PAGE}}`, `{{LABEL_REQUEST}}`, `{{LABELS_EXTRA}}`,
`{{TYPE_TASK}}`, `{{SEAMLEX_CONTACT}}`, `{{CONFIRM_WRITES}}`, `{{DETAIL}}`,
...
```
{{CONF_SPACE}}
├── seamlex-portal-memory                         root index — settings + map of the project's knowledge (Seamlex)
...
`seamlex-portal-memory` page says which page plays which role. **Follow the config page, then this tree,
never a guess.** When the config page does not name a section that a write needs — there is no
`{{CONF_PARENT}}` — say so and ask where it goes; do not create the section on your own initiative.
...
| **Titles are `<Type> <KEY> — <exact Jira summary>`** — `Minuta ABC-12 — <summary>`, `Transcript ABC-12 — <summary>`. The Jira key in upper case, then an em dash, then the Jira wording even when a better title came up. | The title is the only handle a page has: `title ~ "ABC-12"` finds every page of a task, the prefix tells the kinds apart, the summary keeps it readable from the board. |
...
| **Children go under their parent**: minutas under `{{CONF_PARENT}}`, transcripts under their minuta. | `ancestor =` searches and `getConfluencePageDescendants` depend on it. |
| **The body is in `{{LOCALE}}` and in business terms**; platform vocabulary lives only in a page's closing *Para el equipo de delivery* section. | The customer reads these pages; the `business-analysis` rule applies on the page as in the room. |
...
find it. `seamlex-portal-memory` is the **root** index — it also carries the settings — and every
section parent (`{{CONF_PARENT}}`) is a **section** index in the
same shape without the settings. The full body is in `references/memory-template.md`; the shape:
...
2. **Settings** — root page only. A two-column table, one setting per row, key exactly as the commands
   name it (`JIRA_PROJECT`, `CONF_PARENT`, `CONF_SCOPE_PAGE`, `TYPE_TASK`,
...
when it cannot (the `seamlex-portal-memory` page is Seamlex's — **never edit it**), it says which row
Seamlex should add.
...
the prefix (`Minuta`, `Transcript`) tells the kinds apart; ancestor walks a
...
| Looking for | CQL |
|---|---|
| The root index | `space = "{{CONF_SPACE}}" AND title = "seamlex-portal-memory"` |
| A section index | `space = "{{CONF_SPACE}}" AND title = "<section title as the root index lists it>"` |
| Every page tied to a task | `space = "{{CONF_SPACE}}" AND title ~ "<JIRA-KEY>"` — the key upper-cased as it is in the title; the prefix of each title tells minuta from transcript |
...
7. **Close the loop on the index**: when a page was created under a section whose index Claude may
   edit, add the row; otherwise name the row for Seamlex to add. Never edit `seamlex-portal-memory`.
...
|---|---|
| `references/memory-template.md` | The body of an index page — the root `seamlex-portal-memory` with its settings table, and a section index — with a filled example row. |
